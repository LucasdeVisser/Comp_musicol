...
### *NEW* Are there clear tempo differences between albums?
...
```{r}
corpus %>%
  ggplot(aes(x=tempo)) + geom_histogram(fill='red', alpha=0.2) +theme_classic() +geom_density(aes(y=..count..*10, color='red'), size=1) + theme(legend.position = "none", axis.title.y = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank(), axis.line.y=element_blank())
```
